...
void triangle(Vec3f *pts, Vec2f *tex_coords, float *zbuffer, TGAImage &image, TGAImage &texture,TGAImage &glowTexture,TGAImage &normale, Vec3f light_dir,float intensities[3], int width) {
    Vec2i boxMin, boxMax;
    for (int i = 0; i < 3; i++) {
    ...
    for (P.x = boxMin.x; P.x <= boxMax.x; P.x++) {
        for (P.y = boxMin.y; P.y <= boxMax.y; P.y++) {
            ...
            if (zbuffer[idx] < P.z) {
                ...
                TGAColor baseColor = normale.get(tex_x, texture.get_height() - tex_y);
                Vec3f normal(
                        (baseColor.r / 255.0f) * 2 - 1,
                        (baseColor.g / 255.0f) * 2 - 1,
                        (baseColor.b / 255.0f) * 2 - 1
                    );
                TGAColor glowColor = glowTexture.get(tex_x, glowTexture.get_height() - tex_y);
                float intensity = normal * light_dir;
                float glowIntensity = (glowColor.r + glowColor.g + glowColor.b) / (0.2 * 255.0); 
                intensity = std::max(0.0f, std::min(1.0f, intensity));
                baseColor = texture.get(tex_x, texture.get_height() - tex_y);
                //float shadow = 0.3 + 0.9*(shadowbuffer[int(P.x+Pm.y*width)] >= P.z);
                baseColor = ( baseColor + (glowColor*glowIntensity) )* intensity; //* shadow;
                image.set(P.x, P.y, baseColor);
            }
        }
    }
}
...
int main(int argc, char** argv) {
	...
	TGAImage normale;
    normale.read_tga_file("obj/diablo3_pose_nm.tga");
    TGAImage glowTexture;
    glowTexture.read_tga_file("obj/diablo3_pose_glow.tga");
	...
	for (int i = 0; i < model->nfaces(); i++) {
        ...
        triangle(screen_coords, tex_coords, zbuffer, image, texture,glowTexture, normale, light_dir, intensities, image.get_width());
